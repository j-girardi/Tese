...
def multicast_config():
    ...
    buffsize = sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 34603008) #aumento do tamanho do buffer de recepcao para 33MB
    print(f'buffer after: {buffsize}')
...
def mongo_config():
    ...
    db = client.esp_data
    global posts
    posts = db.testedanoite


def data_split():
    if address[0] not in dic_backup:
        dic_backup[address[0]] = data.decode()
        print(dic_backup)
    ...
    if data != dic_backup[address[0]]:
        ...
        if (c%5000== 0):
            ...
            if maxtime < x:
                maxtime = x
            print('Data posted! ..... time taken = {} s'.format(x))
            print(f'Average time = {average} ..... Maximum time taken = {maxtime}')
            #file.write(' {} Data posted ... time taken = {} s\n'.format(c,x))
            print(f'{lostPackets} Packets lost! ..... {timeout} Timeouts')
            post = {
    ...
    while True:
        ...
        while True:
            try:
                ...
                timeout += 1
                print(f'Number of timeouts: {timeout}')
                if c % 5 == 0:
                    break
            else:
